                ...
                <tr key={p.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="flex justify-end gap-1">
                      <button onClick={() => { setEditing(p); setForm(p); setOpen(true); }} className="h-8 w-8 grid place-items-center rounded-lg hover:bg-muted admin-tap"><Edit2 className="h-3.5 w-3.5"/></button>
                      <button onClick={() => confirm(`Delete "${p.code}"?`) && del.mutate(p.id)} className="h-8 w-8 grid place-items-center rounded-lg hover:bg-muted admin-tap text-rose-600"><Trash2 className="h-3.5 w-3.5"/></button>
                    </div>
                  </td>
                </tr>
              ))}
