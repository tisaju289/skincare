      ...
      <aside className="relative w-full max-w-md bg-background h-full flex flex-col shadow-2xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex items-center gap-2">
                  <div className="flex items-center border border-border rounded-full">
                    <button onClick={() => setQty(i.slug, i.quantity - 1)} aria-label="Decrease" className="h-8 w-8 grid place-items-center">
                      <Minus className="h-3.5 w-3.5" />
                    </button>
                    <span className="w-8 text-center text-sm font-bold">{i.quantity}</span>
                    <button onClick={() => setQty(i.slug, i.quantity + 1)} aria-label="Increase" className="h-8 w-8 grid place-items-center">
                      <Plus className="h-3.5 w-3.5" />
                    </button>
                  </div>
                  <button onClick={() => remove(i.slug)} aria-label="Remove" className="h-8 w-8 grid place-items-center text-muted-foreground hover:text-destructive">
        ...
        <div className="border-t border-border p-4 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            {subtotal >= 999 ? "You get free delivery 🎉" : "Add ৳" + (999 - subtotal) + " more for free delivery"}
          </p>
          <Link
            to="/checkout"
            onClick={onClose}
